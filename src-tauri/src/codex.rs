...
pub(crate) struct RawHistoryIndexPage {
    cursor: Option<String>,
    hints: Vec<HistoryTurnHint>,
}

pub(crate) fn text_metrics(text: &str) -> (usize, usize) {
    if text.is_empty() {
        return (0, 0);
    }
    (
...
pub(crate) fn item_text_metrics(item: &Value) -> (usize, usize) {
...
pub(crate) fn project_history_turn_hint(turn: &Value) -> HistoryTurnHint {
    let items = turn
        .get("items")
        .and_then(Value::as_array)
        .cloned()
...
pub(crate) fn history_index_request_active(state: &CodexState, request_id: &str) -> bool {
    state
...
pub(crate) async fn build_external_thread_history_index(
    state: &CodexState,
    thread_id: &str,
    source_version: &str,
    page_size: usize,
...
#[tauri::command]
#[specta::specta]
pub(crate) async fn codex_default_profile_thread_index(
    thread_id: String,
...
#[tauri::command]
#[specta::specta]
pub(crate) fn codex_default_profile_thread_index_cancel(
    request_id: String,
    state: State<'_, CodexState>,
) -> Result<(), String> {
    state
