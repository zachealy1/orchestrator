...
| Reporting | Asset IDs, separate installer/update counts, removed/replaced assets, CSV; download reporting contains no app telemetry | Automated tests plus workflow rehearsal required |
| Installation activity | Stable UUID, daily UTC deduplication, offline retries, opt-out cancellation, public release configuration | Native/frontend tests passed; four production dashboard tiles saved and SQL validated on 2026-09-16; native live ingestion with `environment=test` returned expected distinct counts 1/2/3, including replayed deliveries |
