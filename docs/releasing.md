...
For beta.2, the maintainer reported manual testing on 2026-09-12 and requested no additional application-testing campaign or release rehearsal. Keep the repository's existing required CI checks. A manual dispatch selects the exact source commit to publish from `main`, after preparation reaches `release` and the release PR merges into `main`. GitHub's existing branch and signing/publishing environment access controls still apply; approval alone is not evidence of testing. The separate notarized workflow retains its own validation policy.
