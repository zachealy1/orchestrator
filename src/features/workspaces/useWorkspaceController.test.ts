...
describe("useWorkspaceController", () => {
  it("keeps selected workspace and list refs synchronized", () => {
    const { result } = renderHook(() =>
      useWorkspaceController({ previewDrawerDefaultWidth: 520 }),
    );
  ...
  it("owns preview geometry and cache state for one app instance", () => {
    const { result } = renderHook(() =>
      useWorkspaceController({ previewDrawerDefaultWidth: 640 }),
    );

    expect(result.current.previewDrawerWidth).toBe(640);
    expect(result.current.previewState.status).toBe("idle");

    act(() => {
      result.current.setPreviewResizing(true);
      result.current.directoryEntriesCache.current.set("root", []);
    });

    expect(result.current.previewResizingRef.current).toBe(true);
