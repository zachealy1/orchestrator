...
describe("App Codex auth", () => {
  ...
  it("restores one-shot Plan Mode and preserves plan intent when setup fails before the Codex turn starts", async () => {
    prepareSignedInRun();
    mocks.codexRpcMock.mockImplementation(
      async (_accountId: number, method: string) => {
        ...
        if (method === "thread/start") {
          return { thread: { id: "thread-plan-retry" } };
        }
        if (method === "turn/start") {
          return { turn: { id: "turn-plan-retry" } };
        }
        return {};
      },
    );
    mocks.prepareBrowserSessionMock.mockRejectedValueOnce(
      new Error("The browser runtime is unavailable."),
    );

    const { user } = await renderApp();
    const planModeButton = screen.getByRole("button", { name: /plan mode/i });
    await user.click(planModeButton);
    await user.type(screen.getByLabelText("Prompt"), "Plan a resilient retry");
    await user.click(screen.getByRole("button", { name: /run codex/i }));
